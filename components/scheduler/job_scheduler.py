...
class JobScheduler:
    ...
    Adhoc path (ADHOC jobs):
        ...
        Scheduler transitions PENDING → RUNNING and dispatches.

    On each trigger the scheduler:
        1. Counts total active devices for the job's segments
           by querying the Discovery DB (devices collection).
    ...
    The scheduler does not know or care who consumes the queues.
    """
    ...
    def _cron_loop(self) -> None:
        while self._running:
            ...
            time.sleep(SchedulerConfig.JOB_CHECK_INTERVAL)
    ...
    def _cron_fired(self, expr: str, now: datetime) -> bool:
        ...
        try:
            ...
            return (now - prev).total_seconds() <= SchedulerConfig.JOB_CHECK_INTERVAL
        except Exception as exc:
    ...
    def _count_devices(self, segments: list[str]) -> int:
        """Count ACTIVE devices for given segments in the Discovery DB."""
    ...
    def _publish_devices(
        ...
        """
        Iterate the Discovery DB cursor for matching ACTIVE devices and
