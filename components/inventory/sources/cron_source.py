...
from shared.config.settings import SegmentConfig, InventoryConfig
...
class CronInventorySource(BaseInventorySource):
    """
    Batch inventory source for standalone (cron) mode.
    ...
    def __init__(
        ...
        self.files_path = Path(InventoryConfig.SECONDARY_FILES_PATH)
    ...
    def _from_primary_db(
        ...
        for region in regions:
            try:
                ...
                records.append(DeviceInventoryRecord(
                    region=region,
                    segment=SegmentType(segment),
                    devices=devices,
